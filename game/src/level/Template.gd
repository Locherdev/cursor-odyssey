...
func _ready() -> void:
	...
	if Globals.connect("game_over", self, "_game_over") != OK: Globals.error_connect(self)
	if Globals.connect("debris_destroyed", self, "_instance_asteroid")  != OK: Globals.error_connect(self)
	if Globals.connect("orb_deleted", self, "_instance_orb")  != OK: Globals.error_connect(self)
