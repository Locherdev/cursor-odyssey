...
func _ready() -> void:
	if Globals.connect("trackname", self, "_change_trackname") != OK: Globals.error_connect(self.name)
	if Globals.connect("screenOutput", self, "_output_to_screen") != OK: Globals.error_connect(self.name)
	if Globals.connect("get_orb", self, "_update_orb_counter") != OK: Globals.error_connect(self.name)
	_update_orb_counter()
...
func _update_orb_counter(total_orbs:int = 0) -> void: orb_label.text = str(total_orbs) + "x"
